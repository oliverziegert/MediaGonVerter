...
func (s *JWTService) ValidateToken(encodedToken string) (*jwt.Token, jwt.MapClaims, *e.Error) {
	...
	if err != nil {
		return nil, nil, e.WrapError(e.AuthCredentialsInvalid, "JWT token validation vaild", err)
	}
	return jwtToken, mapClaims, nil
}

func (s *JWTService) Claims(mapClaims *jwt.MapClaims) (*CustomClaims, *e.Error) {
	cc := &CustomClaims{}
